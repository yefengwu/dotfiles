...
let-env PROMPT_MULTILINE_INDICATOR = (^'C:\Users\xuwei\Scoop\shims\starship.exe' prompt --continuation)
...
let-env PROMPT_COMMAND = { ||
    ...
    ^'C:\Users\xuwei\Scoop\shims\starship.exe' prompt $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}
...
let-env PROMPT_COMMAND_RIGHT = { ||
    let width = (term size).columns
    ^'C:\Users\xuwei\Scoop\shims\starship.exe' prompt --right $"--cmd-duration=($env.CMD_DURATION_MS)" $"--status=($env.LAST_EXIT_CODE)" $"--terminal-width=($width)"
}
